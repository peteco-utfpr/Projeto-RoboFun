...
class Lrta:
    def __init__(self, maxRows, maxColumns, goal, initialState, name = "none"):
        """
    ...
    def move(self):
        ...
        arquivo = open("Matriz-" + self.name + ".txt", "a")
        for i in self.matrix:
            arquivo.write(str(i))
            arquivo.write("\n")
        arquivo.write("-------------------------------------------------------------")
    ...
    def do(self):
        ...
        """
        nextMove = self.move()
        return (nextMove, self.goalPos == State(nextMove[0], nextMove[1]))   
